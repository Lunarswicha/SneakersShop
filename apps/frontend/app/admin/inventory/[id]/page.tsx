  ...
  const updateStock = async (variantId: number, newStock: number) => {
    try {
      ...
      if (response.ok) {
        ...
        fetchProductData(); // Rafraîchir les données
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            {product.brand?.name} • Stock total: {totalStock} unités
          </p>
        </div>
        ...
        {product.variants.length === 0 && (
          <div className="text-center py-12">
            <div className="text-gray-500">Aucune variante trouvée pour ce produit</div>
          </div>
        )}
      </div>
    </div>
