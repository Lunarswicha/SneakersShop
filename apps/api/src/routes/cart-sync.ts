...
router.post('/sync', requireAuth, async (req, res) => {
  try {
    ...
    for (const sessionItem of sessionCartItems) {
      // Pour les articles de session, on doit créer des variantes virtuelles
      ...
      if (product.variants.length > 0) {
        ...
        // Créer une variante virtuelle pour les produits sans variantes
    ...
    // Récupérer le panier complet avec les détails
